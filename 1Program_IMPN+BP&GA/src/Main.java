...
//    public static final String DATA_SET = "D:\\毕业设计\\1数据集\\dataset_5000_1000_3_1_1_uniform_phonetic_0.csv";
    ...
    public static void main(String[] args) throws IOException{
        ...
        main.trainedBP = main.getBPFromTXT();
    ...
    private void evaluation() {
        ...
        System.out.println("IMPN+BP&GA算法的查全率为："+chaquanlv+"，查准率为："+chazhunlv+"，误识别率为："+wushibielv);
    }
...
//        int tail = w-1;
    ...
    //这里使用训练好的遗传BP神经网络去判断
    private boolean judgeEqual(People p1, People p2) {
        ...
        double[] tempResult = this.trainedBP.computeOut(tempIn[0]);
    ...
    //计算隐含层节点数
    private int getHiddenLayerNumber(int inputLayerNumber) {
        double N = (double) inputLayerNumber;
        double K = 1.0;
    ...
    //从txt读取训练好的BPDeep
    private BPDeep getBPFromTXT() {
        float[] weightAndThresholdFromTXT = FileIO.readTxtFile(FILEPATH, FILENAME);
        int inputLayerNumber = this.fields.size()-1;
        int hiddenLayerNumber = getHiddenLayerNumber(inputLayerNumber);
...
//                return (o1.getKey()).toString().compareTo(o2.getKey());
    ...
    private void generateKey1() {
        for(People people : this.dataset) {
            ...
            int keyindex0 = this.fields.indexOf(this.sortedFieldsByDifference.get(1));
            int keyindex1 = this.fields.indexOf(this.sortedFieldsByDifference.get(2));
            int keyindex2 = this.fields.indexOf(this.sortedFieldsByDifference.get(4));
            String s0 = people.getAttributeByIndex(keyindex0);
            if(s0 == null) {
                this.indexOfTuplesWithIncompleteSortKey.add(this.mapOfDatasetAfterSort.get(people));
            }
    ...
    private void generateKey2() {
        for(People people : this.dataset) {
            ...
            int keyindex1 = this.fields.indexOf(this.sortedFieldsByDifference.get(3));
            int keyindex2 = this.fields.indexOf(this.sortedFieldsByDifference.get(4));
            String s0 = people.getAttributeByIndex(keyindex0);
            if(s0 == null) {
                this.indexOfTuplesWithIncompleteSortKey.add(this.mapOfDatasetAfterSort.get(people));
